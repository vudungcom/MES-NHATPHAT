...
public class PartBaviaService
{
    ...
    public async Task<List<PartBaviaStep>> GetAllByPartAsync(int partId)
    {
        return await _db.PartBaviaSteps
    ...
    public async Task<long> AddAsync(int partId, PartBaviaStep newStep, int userId, string reason)
    {
        ...
        var maxOrder = await _db.PartBaviaSteps
            .Where(s => s.PartId == partId)
        ...
        _db.PartBaviaSteps.Add(newStep);
    ...
    public async Task UpdateAsync(long stepId, PartBaviaStep updated, int userId, string reason)
    {
        ValidateReason(reason);
        await EnsurePermissionAsync(userId);
