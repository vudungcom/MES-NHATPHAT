...
/// <summary>
/// Service quản lý bảng công đoạn "Quy trình Taro" (PartTaroSteps).
...
public class PartTaroService
{
    ...
    public async Task<PartTaroStep?> GetByIdAsync(long stepId)
    {
        return await _db.PartTaroSteps
            .Include(s => s.Part)
            .Include(s => s.CreatedByUser)
    ...
    public async Task<long> AddAsync(int partId, PartTaroStep newStep, int userId, string reason)
    {
        ...
        {
            newStep.StepOrder, newStep.NC, newStep.StepName, newStep.StandardTime
        });
